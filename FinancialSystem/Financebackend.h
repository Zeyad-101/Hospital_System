...
public:
	...
	vector<Financialdata> loadAllBills() {
		vector<Financialdata> allBills;

		ifstream file(fileName);

		if (!file.is_open()) {
			cerr << "Notice: No existing records found." << endl;
			return allBills;
		}
		...
		while (getline(file, line)) {
			stringstream ss(line);
			string tempStr;
			Financialdata currentBill;

			getline(ss, tempStr, '|');
			currentBill.billId = stoi(tempStr);

			getline(ss, tempStr, '|');
			currentBill.PatientId = stoi(tempStr);

			getline(ss, currentBill.serviceDetails, '|');

			getline(ss, tempStr, '|');
			currentBill.totalAmount = stod(tempStr);

			getline(ss, tempStr, '|');
			currentBill.insuranceCoverage = stod(tempStr);

			getline(ss, tempStr, '|');
			currentBill.issuedate = stoll(tempStr);

			allBills.push_back(currentBill);
		}
...
private:
	...
	string hideCardNumber(const string& cardToHide) {
		if (cardToHide.length() < 6) return "Invalid Card";

		string masked = "";
		for (size_t i = 0; i < cardToHide.length() - 4; ++i) {
			masked += "*";
		}
		masked += cardToHide.substr(cardToHide.length() - 4);
		return masked;
	}
public:
	void setCardDetails(const string& number, const string& holder, const string& expiry, const string& cvvCode) {
		cardNumber = number;
	...
	void displayhideCard() {
		cout << "Card Holder: " << cardHolderName << endl;
		cout << "Card Number: " << hideCardNumber(cardNumber) << endl;
		cout << "Expiry Date: " << expiryDate << endl;
	}

	bool processTransaction(double amount) {
		cout << "\n--- Initiating Secure Transaction ---" << endl;

		if (cardNumber.length() < 16 || cvv.length() < 3) {
			cerr << "Transaction Failed: Invalid card details." << endl;
			return false;
		}

		displayhideCard();
		cout << "Processing charge of $" << amount << "..." << endl;
		cout << "Transaction Approved by Bank!" << endl;
		cout << "-------------------------------------\n" << endl;
...
class Reporting {
public:
	void generateReport(const vector<Financialdata>& bills) {
		double totalRevenue = 0.0;
		double totalInsurance = 0.0;
		for (const auto& bill : bills) {
			totalRevenue += bill.totalAmount;
			totalInsurance += bill.insuranceCoverage;
		}
		cout << "\n--- Financial Report ---" << endl;
		cout << "Total Revenue: $" << totalRevenue << endl;
		cout << "Total Insurance Coverage: $" << totalInsurance << endl;
		cout << "Net Revenue: $" << (totalRevenue - totalInsurance) << endl;
	}
};
...
public:
	double getCoverage(const string& provider) {
		...
		else if (provider == "Metlife") return Metlife;
		else if (provider == "Allianz") return Allianz;
		else {
			cerr << "Warning: Unknown insurance provider. Defaulting to 0% coverage." << endl;
			return 0.0;
		}
	}
};
...
public:
	double getBasePrice(const string& clinicName) {
		...
		else if (clinicName == "Dermatology") return 600.00;
		else if (clinicName == "Cardiology") return 1000.00;
		else {
			cerr << "Warning: Unknown clinic." << endl;
			return 0.0;
		}
	}
};
...
class logs {
public:
	void logTransaction(const Financialdata& bill, bool success) {
		ofstream logFile("transaction_logs.txt", ios::app);
		if (!logFile.is_open()) {
			cerr << "Error: Could not open log file." << endl;
			return;
		}
		logFile << "Timestamp: " << time(0)
			<< " | Bill ID: " << bill.billId
